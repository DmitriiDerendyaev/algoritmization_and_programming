...
void qsort(int* arr, int left, int right)//объявление подфункции сортировки
{
    ...
    while (i <= j)//цикл выполняется, пока границы не сомкнутся
    {
        ...
        if (i <= j)
        {
            ...
            i++; j--;//уменьшнеие границ диапазона
        }//свекла, картофель, лук, сельдь - филе, майонез; 1)Рыба, 2)Лук, 3)Майонез, 4)Картофель - крупная(с палец, 2х хватит), 5)Майонез, 6)Свекла+мазик(3 ложки)

    };

    if (left < j) qsort(arr, left, j);//если обнаружено нарушение порядка в одной из частей, вызывается функция сортировки определенной части
